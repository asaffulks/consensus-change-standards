...
# (theme label, section, trigger lines) -- lines kept short enough to sit inside
# the row with margin to spare (the longest were over-running the box before).
ROUTES = [
    ("Negligence /\nneg. misrep.", "§4.1",
     "Releasing activation code with known bugs and promoting its adoption;\n§552 routes around the economic-loss rule (Biakanja on contested facts)."),
    ("Tortious\ninterference", "§4.2",
     "A proponent with documented economic exposure who pushes a splitting change —\nespecially if the promotion looks instrumental rather than principled."),
    ("Fiduciary\nduty", "§4.3",
     "Unsettled, but Tulip Trading (UK Court of Appeal, 2023) confirmed the question is\njusticiable (claim discontinued 2024); the developer's process determines it."),
    ("Pool\noperators", "§4.4",
...
ax.set_xlim(0, 10); ax.set_ylim(0, 9.9); ax.axis("off")

ax.text(5.0, 9.65, "The legal-exposure landscape", ha="center", va="top",
        fontsize=11, fontweight="bold", color=INK)
ax.text(5.0, 9.05, "When a negligently activated change splits the chain and people lose money,\nmore parties are exposed than most realize:",
...
for i, (theme, sec, trig) in enumerate(ROUTES):
    y = y0 - i * row_h
    ax.add_patch(Rectangle((0.12, y - row_h + 0.15), 9.76, row_h - 0.16,
                           facecolor=AMBER, alpha=0.12, edgecolor=EDGE, lw=0.6))
    ax.add_patch(Rectangle((0.12, y - row_h + 0.15), 0.10, row_h - 0.16,
                           facecolor=AMBER, alpha=0.9, edgecolor="none"))  # left accent
    ax.text(0.42, y - row_h / 2 + 0.07, theme, ha="left", va="center",
            fontsize=8.6, fontweight="bold", color=INK)
    ax.text(2.42, y - row_h / 2 + 0.07, sec, ha="left", va="center",
            fontsize=8.0, color="#9c6a1d")
    ax.text(3.08, y - row_h / 2 + 0.07, trig, ha="left", va="center",
            fontsize=7.0, color=INK)

# green defense footer bar
yb = y0 - len(ROUTES) * row_h
ax.add_patch(FancyBboxPatch((0.12, yb - 0.74), 9.76, 0.82,
