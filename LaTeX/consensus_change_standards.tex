...
This paper occupies the operational and legal layer of an emerging body of work on blockchain governance\,---\,a literature that includes De Filippi and Wright on code as a mode of regulation, the on-chain-versus-off-chain governance debate (Buterin; Zamfir), and the textbook account of off-chain coordination in Narayanan and colleagues. It is most useful when read alongside \textit{BCAP}, not in place of it.

\subsection*{Governance models in adjacent open protocols}

The framework's central move\,---\,no activation until running, independently reviewed code has cleared minimum maturity gates\,---\,is not new to open-protocol governance, though it is new to Bitcoin consensus change. It is the operating premise of the Internet's own standards process. RFC 7282 states the IETF credo as ``rough consensus and running code,'' an expressly anti-majoritarian formulation that, like this framework and like \textit{BCAP}, rejects head-counting as a substitute for technical readiness. RFC 2026, the Internet Standards Process, makes the maturity bar concrete: a specification advances only on ``multiple, independent, and interoperable implementations from different code bases'' together with ``substantial operational experience.'' That is the running-reviewed-code-before-activation gate restated in the vocabulary of the body that invented it.

The lineage is documented, not inferred. EIP-1 records that it is ``derived heavily from Bitcoin BIP-0001 written by Amir Taaki which in turn was derived from Python PEP-0001''\,---\,a PEP-1 to BIP-1 to EIP-1 descent tracing back to the IETF's request-for-comments practice. Bitcoin's proposal system is a branch of that tree, and the standards from which the present framework's gates descend are its older relatives. The contrast at the far end of that tree is instructive: Ethereum runs a frequent-coordinated-upgrade cadence through its All Core Devs calls, a process built to ship hard forks on a regular schedule, whereas Bitcoin's conservatism leaves it without any comparable maturity discipline\,---\,which is the gap this framework fills.
...
\begin{itemize}
  ...
  \item \textbf{Below 60\%} (BIP-110's 55\%): reckless. About three in ten chance of a six-block reorg during activation. At 95\%, the same chance is effectively zero\,---\,two parts in a hundred million.
\end{itemize}
...
The trial never took place. Tulip Trading discontinued the claim in 2024, after the \textit{COPA v Wright} litigation rejected Dr.~Wright's claim to be Satoshi Nakamoto\,---\,the claim on which Tulip's asserted ownership of the relevant bitcoin depended. The discontinuance leaves the Court of Appeal's holding intact and the duty question untried. The case thus stands not for the proposition that developers owe such duties, but for the proposition that the question is justiciable on appropriate facts.
...
\begin{itemize}
  \item \textbf{100\%: Green.} The proposal has met all applicable minimum standards and is ready for activation signaling.
...
\noindent\textbf{Score: Taproot 17/17 applicable (Green). BIP-110 3/20 (Red). SegWit2x ${\approx}6/17$ applicable (Red; scored in prose below).}
...
The answer is to distinguish what the coordination is organized around. Coordinating around published, challengeable criteria is not equivalent to coordinating around private preference or insider pressure. The criteria are visible; a stakeholder who invokes them exposes the basis of the refusal to scrutiny, and anyone who disagrees can contest the scoring on the record. The framework binds no one, certifies nothing, and confers no veto; its ``decline to recognize'' step describes an option economic nodes already possess, not an instruction they are obliged to follow. A centralizing instrument concentrates the power to decide; this framework distributes the power to argue. The two are not the same, and the difference is the whole point. The same design choice keeps the coordination on the right side of the antitrust line (\sect{5.2}): published criteria applied unilaterally are information. Only agreement would make them something else.

\section{``This framework is a chilling instrument that will deter good-faith development.''}

This is the strongest objection a critic can level at the paper itself, and it is sharpest precisely because the author is a practicing litigator. The objection runs: a document that pairs a worked tort map against developers (\sect{4.1}) with a scorecard whose Red finding \sect{5.2} calls ``the exposure'' is not a neutral description of risk. It is an instrument of intimidation. Its predictable effect is to chill the open-source contribution Bitcoin depends on, because a contributor who reads it will reasonably fear that any change carrying their name could be turned into a damages theory. Worse, an adversary could weaponize the pairing\,---\,publish a self-serving low score against a disfavored developer's proposal and wave \sect{4.1} as the threatened consequence\,---\,converting a tool of evaluation into a tool of harassment.

The objection has real force, and the answer is not to deny the teeth but to locate them precisely. First, the liability theories Chapter~4 maps are not the paper's invention. Negligent misrepresentation, tortious interference, and the duty questions left open by \textit{Tulip Trading} exist in the law whether or not anyone writes them down; Chapter~4 describes the terrain, it does not lay the mines. A contributor faces exactly the exposure the law already assigns\,---\,the paper changes the contributor's information, not the contributor's risk. Second, and decisively, Section-3 compliance is a complete defense. A change that documents its review, meets its threshold, and clears the floors scores green and carries no exposure the framework can manufacture; the net effect on a good-faith contributor is therefore protective, supplying a documented standard of care to point to rather than a trap to fall into. Third, the deterrence the paper theorizes (\sect{4.0}) runs against one thing only: shipping a change past a documented Red finding\,---\,against recklessness, never against contribution, and never against non-adoption. Good-faith coding that turns out mistaken is not its target; rushing a flagged change to forced activation is. Fourth, the framework deliberately aims liability at identifiable campaign actors\,---\,pools, deploying platforms, conflicted promoters who push past the red flags\,---\,not at pseudonymous coders contributing in good faith. The adversary who tries to weaponize a manufactured score inherits the framework's own discipline: the score is challengeable on the record (\sect{6.7}), and a score unsupported by the underlying facts documents the harasser, not the target.
...
\begin{itemize}
  ...
  \item \textit{``This framework is a chilling instrument that will deter good-faith development.''} The liability theories pre-exist the paper; Section-3 compliance is a complete defense; the deterrence runs only against shipping past a documented Red finding, never against good-faith contribution or non-adoption, and it points at identifiable campaign actors, not pseudonymous coders (\sect{6.8}).
\end{itemize}
\end{plainenglish}
...
\reqitem{B.1}{Recognition policy (exchange / custodian).} ``[Operator] evaluates proposed changes to the Bitcoin consensus rules against the Consensus Change Readiness Checklist (Fulks, \textit{Consensus Change Standards} (4th ed.~2026), \sect{5.1}) as one input to its independent listing, custody, deposit-crediting, and contract-denomination decisions. A proposal that has not been publicly scored, or that scores below the Yellow band, is presumptively not recognized as `Bitcoin' for those purposes until the deficiencies are remediated and a revised public scoring supports recognition. This policy is adopted and applied unilaterally; it neither requires nor contemplates agreement or coordinated action with any other market participant.''
...
\begin{description}[leftmargin=0pt, labelindent=0pt, style=nextline, itemsep=8pt]
  ...
  \item[UASF (User-Activated Soft Fork).] A soft fork activated by economic nodes enforcing new rules without depending on a miner-signaling threshold being met. BIP-148 is the canonical example. UASF works only when prior community support is overwhelming; UASF without that support yields the BIP-110 outcome. See \sect{3.4}.
...
\begin{itemize}[leftmargin=1.5em, itemsep=4pt]
  ...
  \item Buterin, Vitalik. ``Notes on Blockchain Governance.'' 2017.
  ...
  \item Zamfir, Vlad. ``Against On-Chain Governance.'' 2017.
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em, itemsep=4pt]
  ...
  \item Warsaw, Barry, Jeremy Hylton, David Goodger, and Nick Coghlan. \textit{PEP Purpose and Guidelines}. PEP 1, Python Software Foundation, 2000 (rev.). \url{peps.python.org/pep-0001}
