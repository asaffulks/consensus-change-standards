...
The framework's bite does not depend on the direction of the change. A liberalizing proposal advanced on the same terms would score the same way: a soft fork that loosened an existing standardness or script limit, released with an unreviewed client, a 55\% activation threshold, a six-week timeline, and no chain-split contingency plan, would fail the same Code Quality, Activation Safety, and Community Process criteria and land in the same Red band\,---\,for reasons none of which reference whether the change expands or contracts what Bitcoin permits. The scorecard measures readiness, not the merits of the underlying change, and it measures readiness identically in both directions.
