...
- **Second Edition v2.6 (May 2026)** — current canonical PDF: `consensus_change_standards_v2.6.pdf` at repo root. Built from LaTeX (`LaTeX/consensus_change_standards.tex`). This is the deliverable PDF going forward. **v2.6 adds**, beyond v2.5:
  ...
  - **Layout polish (2026-05-24, commits `36012b0`, `235f1df`, `4ddd67f`):** §5.4 longtable cleaned up — criterion-7 cell rewording (`unit/integration/regression` → `unit, integration, regression`) to match §5.1 worksheet wording and eliminate 13.3pt cell overflow at the unbreakable slashes; `\tabcolsep` reduced to 4pt to clear 8.9pt alignment overrun; `>{\raggedright\arraybackslash}` added to all data columns to eliminate mid-syllable hyphenation; `\clearpage` inserted before `\begin{longtable}` so the table starts on its own page (heading + intro sit alone on prior page). Page count unchanged at 66. Pattern documented for any future narrow comparison table — see memory `feedback_table_layout_narrow_cells.md`.
  - **BIP-110 accuracy pass (2026-05-24, commit `af4fc2e`):** §1.3.A previously called BIP-110 a "55% UASF" only. Per bip110.org and the proposal spec, BIP-110 is actually a LOT=true mechanism (55% early-lock-in trigger + mandatory lock-in around August 2026 where BIP-110 nodes reject non-signaling blocks + activation at `max_activation_height` block 965,664). The §3.4 LOT=true critique already applied; this fix connects it to the case study explicitly so a careful reader (Lopp, AJ Towns, Sjors) doesn't flag a gap. §2.5 summary table mechanism column updated to `UASF (Knots, LOT=true)`. §5.4 row B4 BIP-110 cell expanded to `55% UASF + LOT=true mandatory lock-in (Aug 2026); sunset ~1 year after activation`. Verified BIP-110 specifics live: `bip110monitor.com` peak signaling = exactly 0.15% (period 467, 3/2016 blocks — matches paper's "≤0.15%" verbatim); Knots `v29.3.knots20260508` release notes quote "fixes critical vulnerabilities in long-standing network design" verbatim; bitcoindev mailing list post timestamp confirmed Oct 26 2025 12:43 UTC. Page count unchanged at 66. **Current HEAD: `af4fc2e`.**
